...
function HorizontalScoreBar({ label, percent, color, barMaxWidth = 280 }) {
  const p = Math.min(100, Math.max(0, Math.round(Number(percent) || 0)));
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
      ...
      <div style={{ width: 36, fontSize: 10, fontWeight: 800, color }}>{p}%</div>
    </div>
  );
}
...
function ResultPDF({ interpretation, counsellorNote, user, riasecReport, showDiscussionForCounsellor = false }) {
  ...
  const readinessGuidance = useMemo(
    () => filterSecondaryReportLinesRemovingCoreAdvice(coerceStringArray(interpretation?.readiness_action_guidance)),
    [interpretation]
  );
  ...
  const pdfCareerConfidenceExplanationDisplay = useMemo(
    () =>
      interpretation?.career_confidence_explanation
        ? removeCoreAdviceSentencesFromSecondaryText(interpretation.career_confidence_explanation)
        : '',
    [interpretation?.career_confidence_explanation]
  );
  ...
  return (
    <div id="result-pdf" style={styles.container}>
                    ...
                    topThreeRiasecTraits.map((d) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {rc.label}{' '}
                          <span style={{ color: rc.primary }}>{Math.round(d.score || 0)}%</span>
                        </span>
                      );
                    })
                  ) : (
          ...
          >
            ...
            >
              <p style={{ margin: 0, fontSize: 12, color: BRAND_COLORS.text, lineHeight: 1.55 }}>
                ...
                {sortedRiasecByScore
                  .slice(0, 2)
                  .map((d) => d.title || RIASEC_COLORS[d.code]?.label || d.code)
                  .join(' + ')}
                {sortedRiasecByScore[2] ? (
                  <span>
                    . <strong>Also strong:</strong>{' '}
                    {sortedRiasecByScore[2].title ||
                      RIASEC_COLORS[sortedRiasecByScore[2].code]?.label ||
                      sortedRiasecByScore[2].code}
                  </span>
                ) : null}
              </p>
            </div>
            ...
            return (
              <div key={dimension.code} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                  <div>
                    <p style={{ margin: '0 0 4px 0', fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', color: rc.primary }}>
              ...
              >
                ...
                <p style={{ margin: 0, fontSize: 13, fontWeight: 800, color: BRAND_COLORS.primary }}>
                  {interpretation.career_confidence_level
                    ? friendlyCareerDirectionHeadline(interpretation.career_confidence_level)
                    : '—'}
                </p>
              </div>
            </div>
      ...
      {/* Readiness Action Guidance (coerced — API may send JSON string) */}
      {readinessGuidance.length > 0 && (
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>What To Do Next</h2>
          <div style={{
            ...styles.card,
            ...
            <ul style={styles.bulletList}>
              {readinessGuidance.map((action, idx) => (
      ...
      {/* Career Confidence — same gate as ResultPage / CareerConfidence.jsx */}
      ...
      {pathwayPayload?.fieldAnalysis?.valid !== false && pathwayPayload?.fieldAnalysis?.pathwayRows?.length > 0 && (
        <div style={styles.section}>
          <h2 style={styles.sectionTitle}>TOP 3 CAREER PATHWAY</h2>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', gap: 12 }}>
                    ...
                    >
                      <p style={{ margin: 0, fontSize: 24, fontWeight: 900, color: '#0f172a', lineHeight: 1 }}>
                        {row.riasecCode}
                      </p>
                      <p style={{ margin: '4px 0 0 0', fontSize: 11, fontWeight: 700, color: '#334155' }}>
                        {riasecLabels[row.riasecCode] || row.riasecCode}
                      </p>
                      <span
                        style={{
                          ...styles.badge,
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8 }}>
                        {paddedFields(row.fields).map((field, fieldIdx) => {
                          ...
                          return (
                            ...
                            >
                              <div style={{ fontSize: 12, fontWeight: 800, color: '#0f172a', marginBottom: 7 }}>
                                {field.aspiringField}
                              </div>
