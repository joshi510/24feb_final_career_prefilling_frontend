...
function SectionTestFlow({ attemptId, initialSection, onSectionComplete, onComplete }) {
  ...
  const [focusedOptionIndex, setFocusedOptionIndex] = useState(-1); // -1 means no option is focused/selected initially
  ...
  // Debug logging for question loading (disabled in production)
  useEffect(() => {
    ...
    try {
      ...
      if (progress.current_section) {
        ...
        if (nextSection) {
          ...
          // All sections completed (Section 10 done), complete test and automatically show results
          try {
  ...
  const handlePause = async () => {
    ...
    try {
      // Preserve current time BEFORE calling backend
      // This is critical - backend calculation might be wrong, so we preserve what we know is correct
      const currentTimeBeforePause = timer.currentTime;
      console.log('⏸️ Pausing section...', { currentTime: currentTimeBeforePause });
      
      const response = await testAPI.pauseSection(attemptId, currentSection.id);
      console.log('✅ Pause response:', response);
      
      // Use backend value ONLY if it's valid (> 0), otherwise preserve current time
      // Backend calculation can be wrong if total_time_spent is incorrect
      const remainingTime = (response.remaining_time_seconds !== undefined && response.remaining_time_seconds > 0)
        ? response.remaining_time_seconds
        : currentTimeBeforePause; // Always preserve the current time if backend returns invalid value
      
      console.log('⏰ Setting timer to paused:', { remainingTime, isPaused: true, backendValue: response.remaining_time_seconds });
      ...
      // Don't sync with backend immediately - it might override with wrong value
      // The timer is paused, so it won't count down anyway
    } catch (err) {
  ...
  const handleResume = async () => {
    ...
    try {
      // Preserve current time BEFORE calling backend
      // Backend might return wrong values (0 or negative), so we preserve what we know is correct
      const currentTimeBeforeResume = timer.currentTime;
      console.log('🔄 Resuming section...', { currentTime: currentTimeBeforeResume, isPaused: timer.isPaused });
      
      // Clear any existing interval to ensure clean restart
      if (timerInterval) {
        clearInterval(timerInterval);
        setTimerInterval(null);
      }
      ...
      // Always fetch remaining time from backend progress API after resume
      const progress = await testAPI.getTestProgress(attemptId);
      let remainingTime = null;
      
      if (progress && progress.current_section && progress.current_section.id === currentSection.id) {
        remainingTime = progress.remaining_time_seconds;
        console.log('📊 Progress remaining_time_seconds:', remainingTime);
      }
      
      // Use backend value ONLY if it's valid (> 0)
      // If backend returns 0 or negative, it's wrong - use preserved current time
      if (remainingTime === null || remainingTime === undefined || remainingTime <= 0) {
        // Backend value is invalid, try response
      ...
      console.log('⏰ Setting timer to resume:', { remainingTime, isPaused: false });
      
      // Force resume by setting isPaused to false and updating time
      // This will trigger the useEffect to restart the timer interval
      setTimer({
        totalTime: remainingTime,
        isPaused: false, // Force resume - this triggers the timer to restart
        currentTime: remainingTime
      });
      
      // Don't sync with backend after resume - it might return wrong values (0)
      // The timer will count down from the preserved value which is correct
    } catch (err) {
    ...
    try {
      ...
      try {
        ...
        if (progress && progress.answers && Object.keys(progress.answers).length > 0) {
          // Only restore if there are actual saved answers (section was in progress)
          // Convert all answer keys to numbers for consistency
          const normalizedAnswers = {};
          Object.keys(progress.answers).forEach(key => {
            normalizedAnswers[Number(key)] = progress.answers[key];
          });
          setAnswers(normalizedAnswers);
          setCurrentQuestionIndex(progress.current_question_index || 0);
        } else {
          // New section - clear all answers and reset focus
          setCurrentQuestionIndex(0);
          setAnswers({});
          setFocusedOptionIndex(-1); // No option focused by default
        }
      } catch (progressErr) {
  ...
  const parseOptions = (optionsInput) => {
    ...
    if (typeof optionsInput === 'string' && optionsInput.trim()) {
      ...
      // Check if it's pipe-separated (|) or comma-separated (,)
      const separator = trimmed.includes('|') ? '|' : ',';
      
      // Split by separator, but be smart about it - look for pattern ", A)", ", B)", etc.
      // This handles cases where option text might contain commas
      // Use a more robust regex that matches "A) Text" or "A. Text" patterns
      const parts = trimmed.split(new RegExp(`${separator}\\s*(?=[A-E][\\)\\.])`));
      ...
      if (options.length === 0) {
        // Try splitting by separator and looking for letter patterns
        const altParts = trimmed.split(separator).map(p => p.trim());
        ...
        try {
          ...
          if (progress && progress.current_section && progress.status !== 'COMPLETED') {
            ...
            if (restoredSection) {
              ...
              setCurrentQuestionIndex(progress.current_question_index || 0);
              // Normalize answer keys to numbers for consistency
              const restoredAnswers = progress.answers || {};
              const normalizedAnswers = {};
              Object.keys(restoredAnswers).forEach(key => {
                normalizedAnswers[Number(key)] = restoredAnswers[key];
              });
              setAnswers(normalizedAnswers);
              setFocusedOptionIndex(-1); // Reset focus when restoring
  ...
  // Reset focus when question changes (but don't auto-focus to avoid appearing selected)
  useEffect(() => {
    if (currentQuestion) {
      setFocusedOptionIndex(-1); // Reset to -1 so no option appears focused/selected
      setError(''); // Clear any errors when question changes
    }
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden max-w-6xl mx-auto w-full px-2 sm:px-0">
      ...
      <div className="flex-shrink-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg p-3 mb-2 border border-slate-200 dark:border-slate-700 safe-area-top">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-xs sm:text-sm font-medium text-blue-600 dark:text-blue-400">
                Step {currentSection.order_index} of {sections.length || 10}
              </span>
            </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3">
            ...
            {!timeUp && (
              timer.isPaused ? (
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 active:scale-95 transition-all shadow-sm"
                >
                  Resume
                </button>
              ) : (
                <button
                  onClick={handlePause}
                  className="px-3 py-1.5 text-xs font-medium bg-amber-600 text-white rounded-lg hover:bg-amber-700 active:scale-95 transition-all shadow-sm"
                >
                  Pause
                </button>
              )
            ...
            {timeUp && (
              <div className="px-3 py-1.5 text-xs font-medium bg-red-600 text-white rounded-lg shadow-sm">
                Submitting...
            </div>
            )}
            {/* Clock Display */}
      ...
      {error && (
        <div className="flex-shrink-0 mb-2 p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-400 text-xs sm:text-sm">
          {error}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto pr-0 sm:pr-1">
        {loading ? (
          ...
          <motion.div
            ...
            className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-3 sm:p-4 border border-slate-200 dark:border-slate-700 flex flex-col"
          >
            {/* Question Progress */}
            ...
            {/* Question */}
            <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-slate-100 mb-3 flex-shrink-0 leading-relaxed">
              {currentQuestion.question_text}
            </h3>

            {/* Options - All visible, no internal scrolling */}
            <div className="space-y-2 mb-2.5" role="radiogroup" aria-label="Answer options">
              {(() => {
                const availableOptions = getAvailableOptions();
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (!timeUp) {
                        handleAnswer(questionId, option.key || option.value);
                    ...
                    tabIndex={timeUp ? -1 : (isFocused ? 0 : (focusedOptionIndex === -1 && idx === 0 ? 0 : -1))} // First option is tabbable if none focused
                    disabled={timeUp}
                    className={`w-full text-left py-2.5 px-3 rounded-lg border-2 transition-all outline-none ${
                      timeUp
                  ...
                  >
                    <span className="font-medium text-sm sm:text-base text-slate-900 dark:text-slate-100 leading-relaxed">
      ...
      {(() => {
        ...
        <div className="flex-shrink-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg p-3 mt-2 border border-slate-200 dark:border-slate-700 z-10 safe-area-bottom">
          <div className="flex gap-2 sm:gap-3 justify-between">
            <button
                onClick={handlePrevious}
              disabled={currentQuestionIndex === 0 || !currentQuestion}
              className="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-sm bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 min-h-[44px]"
            >
              Previous
            </button>
              {!isLastQuestion ? (
              <button
                ...
                className="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-sm bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 min-h-[44px]"
              >
                Next
              </button>
            ) : (
              <button
                onClick={submitSection}
                  disabled={loading || !currentQuestion || !canSubmit}
                className="flex-1 sm:flex-none px-3 sm:px-4 py-2 text-sm bg-green-600 text-white rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-green-700 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800 disabled:focus:ring-0 min-h-[44px]"
              >
