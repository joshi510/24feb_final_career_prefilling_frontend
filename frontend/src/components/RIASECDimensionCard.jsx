...
function RIASECDimensionCard({ dimension }) {
  ...
  const normalizeMatchLabel = (score, fromApi) => {
    const u = String(fromApi || '').toUpperCase();
    if (u.includes('HIGH')) return 'GREAT FIT';
    if (u.includes('MODERATE')) return 'GOOD FIT';
    if (u.includes('LOW')) return 'SOME FIT';
    return calculateMatchLevel(score);
  };
  ...
  const matchLevel = normalizeMatchLabel(score, dimension.matchLevel);
            ...
            <ul className="space-y-1">
              {dimension.personalizedAnalysis.split('•').filter(item => item.trim()).map((item, index) => (
                <li key={index} className="text-slate-700 dark:text-slate-300 text-sm flex items-start gap-2">
                  ...
                  <span>{simplifyReportText(item.trim())}</span>
                </li>
              ))}
            </ul>
          ) : (
            ...
            <ul className="space-y-1">
              {dimension.coreStrengths.map((strength, index) => (
                <li key={index} className="text-slate-700 dark:text-slate-300 text-sm flex items-start gap-2">
                  ...
                  <span>{simplifyReportText(strength)}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-1">
              {dimension.growthAreas.map((area, index) => (
                <li key={index} className="text-slate-700 dark:text-slate-300 text-sm flex items-start gap-2">
                  ...
                  <span>{simplifyReportText(area)}</span>
                </li>
              ))}
            </ul>
          </div>
