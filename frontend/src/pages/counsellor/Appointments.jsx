...
function Appointments() {
  ...
  const loadAppointments = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const response = await appointmentAPI.getCounsellorAppointments(page, pageSize, statusFilter);
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-xl sm:text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent">Student Appointments</h2>
              <p className="text-slate-600 dark:text-slate-400 mt-1 text-sm sm:text-base">Manage student counselling appointments</p>
            </div>
          </div>

          {/* Sticky Filters Bar */}
          <div className="sticky top-16 sm:top-20 z-20 bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-3 sm:p-4 shadow-lg">
        ...
        {pagination.total_pages > 1 && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-6 mt-6 border-t border-slate-200/50 dark:border-slate-700/50">
            ...
            <div className="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto">
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-3 py-2 min-h-[40px] rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-slate-700 transition-all font-semibold text-sm shadow-sm"
              >
                First
              </motion.button>
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-4 py-2.5 min-h-[40px] rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all font-semibold shadow-sm hover:shadow-md"
              >
                Previous
              </motion.button>
              <span className="col-span-2 sm:col-span-1 text-center px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300">
