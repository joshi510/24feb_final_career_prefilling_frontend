...
function StudentDashboard() {
  ...
  const initializeDashboard = async () => {
    try {
      ...
      if (sectionsData && sectionsData.sections && sectionsData.sections.length > 0) {
        ...
        // BUT: Check if user is already on a result page - don't redirect if they are
        if (sectionsData.can_attempt_test === false) {
          console.log('Test already completed, checking if already on result page...');
          
          // Check if user is currently viewing a result page
          const currentPath = window.location.pathname;
          const isOnResultPage = currentPath.startsWith('/result/');
          
          if (isOnResultPage) {
            // User is already on result page - don't redirect, just prevent test access
            console.log('User already on result page, not redirecting');
      ...
      return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-300">
          ...
          <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-200 dark:border-slate-700 text-center">
  ...
  if (useSections && attemptId && selectedSection) {
    ...
    return (
      <div className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-300 flex flex-col" style={{ minHeight: '100dvh' }}>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-200 text-center">
            <div className="text-green-500 text-4xl sm:text-5xl mb-4">✓</div>
            <h2 className="text-xl sm:text-2xl font-semibold text-slate-900 mb-2">Test Submitted Successfully!</h2>
            <p className="text-sm sm:text-base text-slate-600">Redirecting to your results...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 sm:py-8">
        ...
        <motion.div
          ...
          className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 border border-slate-200"
        >
          <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-slate-900 mb-4 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0">
            <button
              ...
              className="w-full sm:w-auto px-4 sm:px-6 py-2 bg-slate-200 text-slate-700 rounded-lg font-medium hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm sm:text-base"
            >
              Previous
            </button>
            ...
            {currentQuestionIndex === questions.length - 1 ? (
              <button
                ...
                className="w-full sm:w-auto px-4 sm:px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-medium shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
              >
              ...
              <button
                ...
                className="w-full sm:w-auto px-4 sm:px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg font-medium shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 transition-all text-sm sm:text-base"
              >
                Next
              </button>
            )}
