...
export const testAPI = {
  ...
  getRIASECReport: (attemptId) => apiRequest(`/test/riasec-report/${attemptId}`),
...
export const adminAPI = {
  ...
  getStudentResultFull: (studentId, resultId) => {
    // Clean both IDs to remove any colons or invalid characters
    // This is the optimized endpoint that returns all data in one request
    const cleanStudentId = String(studentId).trim().split(':')[0].split('/')[0].split(' ')[0];
    const cleanResultId = String(resultId).trim().split(':')[0].split('/')[0].split(' ')[0];
    return apiRequest(`/admin/students/${cleanStudentId}/result/${cleanResultId}/full`);
  },
  getStudentTestAttempts: (studentId) => {
    // Clean studentId to remove any colons or invalid characters
    const cleanStudentId = String(studentId).trim().split(':')[0].split('/')[0].split(' ')[0];
    return apiRequest(`/admin/students/${cleanStudentId}/test-attempts`);
  },
  ...
  uploadExcelQuestions: (file) => {
    const formData = new FormData();
    formData.append('file', file);
    
    const token = authService.getToken();
    const API_BASE = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8001';
    const url = `${API_BASE}/admin/questions/upload-excel`;
