...
/** career_confidence_level from API or RIASEC engine (HIGH / MODERATE / LOW) */
export function friendlyCareerDirectionHeadline(level) {
  const u = String(level || '').toUpperCase();
  if (u === 'HIGH') return 'Clear interest direction';
  if (u === 'MODERATE') return 'Multiple interests present';
  return 'More exploration needed';
}
