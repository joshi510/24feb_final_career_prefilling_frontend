...
function StudentsList() {
  ...
  const pagination = studentsResponse?.pagination || {
    total_records: Array.isArray(studentsResponse) ? studentsResponse.length : students.length,
    total_pages: 1,
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // Use students array directly (already filtered by backend)
  ...
  }, [focusedRowIndex, pagination, students]);

  const handleViewResult = (student) => {
    // Handle both student object and studentId for backward compatibility
    const studentData = typeof student === 'object' ? student : students.find(s => s.id === student);
  ...
  if (error && students.length === 0) {
    return (
      <AdminLayout>
        <Error message={error} onRetry={() => refetchStudents()} />
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">Students</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
              whileTap={{ scale: 0.95 }}
            >
        ...
        <motion.div
          ...
          className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 sm:p-4 shadow-sm"
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {/* Search */}
            <div className="md:col-span-2 relative">
              <IconSearch className="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-slate-400" />
              <input
                type="text"
                placeholder="Search by name, email, or mobile..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-2.5 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
              />
            </div>
        ...
        >
              ...
              <div className="px-3 sm:px-4 py-8 sm:py-12 text-center">
                ...
                >
                  <div className="space-y-2 sm:space-y-3">
                    <div>
                      <div className="text-sm sm:text-base font-semibold text-slate-900 dark:text-slate-100 mb-1">
                        {student.full_name}
                      </div>
                      <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                        {student.email}
          ...
          {infiniteScroll && !loading && pagination.current_page >= pagination.total_pages && students.length > 0 && (
            <div className="px-4 lg:px-6 py-4 bg-slate-50 dark:bg-slate-700/30 border-t border-slate-200 dark:border-slate-700 text-center">
