...
export const LOW_RAW_TOTAL_THRESHOLD = 42;
/** No dimension above this (raw) → weak */
export const LOW_MAX_DIMENSION_RAW = 8;
/** Normalized spread below this → “flat” profile */
export const FLAT_PROFILE_SPREAD = 0.03;
/** Dominant normalized share above this → High confidence */
export const CONFIDENCE_DOMINANT_STRONG = 0.35;
...
/**
 * Centralized display policy: rows can force a field to appear in top pathway columns.
 * This affects field-card visibility only; scoring remains unchanged.
 */
const PATHWAY_ROW_FIELD_POLICY = {
  I: { mustIncludeField: 'Networking' },
  A: { mustIncludeField: 'Humanities' }
};
...
function isFlatNormalizedProfile(normalized) {
  ...
  return mx - mn < FLAT_PROFILE_SPREAD;
}
...
export function assessConfidence(state, raw, flatProfile, veryLowEngagement) {
  const domShare = state.normalized[state.dominantCode] || 0;

  if (veryLowEngagement) {
    return {
      confidence: 'Low',
      message: 'Not enough strong preferences detected. Consider retaking the assessment or discussing with a counsellor.'
    };
  }
  ...
  if (flatProfile) {
    return {
      confidence: 'Low',
      message:
        'Your scores are very similar across dimensions, so rankings use a neutral order. Exploring a few different pathway types may help clarify what fits you best.'
    };
  }

  if (domShare > CONFIDENCE_DOMINANT_STRONG) {
    return { confidence: 'High', message: null };
  }
...
export function getFieldRecommendations(dimensions, options = {}) {
  ...
  const veryLowEngagement =
    totalRaw < LOW_RAW_TOTAL_THRESHOLD || (maxDim < LOW_MAX_DIMENSION_RAW && totalRaw < LOW_RAW_TOTAL_THRESHOLD * 1.5);
